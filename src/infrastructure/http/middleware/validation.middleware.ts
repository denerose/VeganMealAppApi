...
export const sanitizeString = (input: string, maxLength = VALIDATION_RULES.STRING_MAX_LENGTH): string => {
...
export const validateEmail = (input: string): boolean => {
  return VALIDATION_RULES.EMAIL_PATTERN.test(input) && input.length <= VALIDATION_RULES.EMAIL_MAX_LENGTH;
};

export const validatePagination = (limit?: string, offset?: string): { limit: number; offset: number } => {
